...
call mvn jtest:jtest -Djtest.config="jtest.dtp://Metrics" -Dmaven.test.skip=true -Dmaven.test.failure.ignore=true -Dproperty.dtp.project=%DTP_PROJECT% -Djtest.showsettings=true -Djtest.report=%REPORT_DIR%\jtest-ma -Dproperty.console.verbosity.level=high -Dproperty.build.id=%BUILD_ID% > %LOG_DIR%\jtest-ma-%RUN_TIME%.log 2>&1
